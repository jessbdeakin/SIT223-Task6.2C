	...
	stages {
		...
		stage('Unit and Integration Tests') {
			steps {
				...
				mail to: "${EMAIL_ADDRESS}", subject: "[JENKINS] Successful test", body: "${BUILD_LOG}"
			}
			post {
				failure {
					mail to: "${EMAIL_ADDRESS}", subject: "[JENKINS] Failed test", body: "${BUILD_LOG}"
				}
			}
		}
		...
		stage('Security Scan'){
			steps {
				...
				mail to: "${EMAIL_ADDRESS}", subject: "[JENKINS] Successful security scan", body: "${BUILD_LOG}"
			}
			post {
				failure {
					mail to: "${EMAIL_ADDRESS}", subject: "[JENKINS] Failed security scan", body: "${BUILD_LOG}"
				}
			}
		}
